...
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint16_t p, neoPixelType t) :
  ...
  //顺序依次为：RGB灯控制脚，灯号（0 全亮），R值，G值，B值
  void Adafruit_NeoPixel::colorWipeSingle(uint8_t location, uint8_t Red, uint8_t Green, uint8_t Blue)
  {
	uint32_t color;
	this->begin();
	this->clear();
  //  strip.show(); // Initialize all pixels to 'off'
	...
	color=this->Color(Red,Green,Blue);
	if((location>=1)&&(location<=4))
	{
	  this->setPixelColor((location-1), color);
	  this->show();
	}
	else if(location==0)
	{
